...
public class ProductModelCreationListener implements ServletContextAttributeListener {
	...
	public void attributeAdded(ServletContextAttributeEvent scae) {
		...
		if (value instanceof ProductModel) {
			...
			scae.getServletContext().log(String.format("Product model is created at %s",
					LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))));
			scae.getServletContext().log(String.format("Attribute name is %s", name));

			ProductModel productModel = (ProductModel) value;
			scae.getServletContext().log(String.format("Product count is %s", productModel.getList().size()));
		}
	}
}
