package com.example.springrestdemo1.exception;
...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        ...
        final ValidationErrorDTO dto = processFieldErros(fieldErrors);
    ...
    public final ResponseEntity<Object> handleBadRequest(final RuntimeException ex, final WebRequest request) {
        final ApiError apiError = message(HttpStatus.BAD_REQUEST, ex);
        return handleExceptionInternal(ex, apiError, new HttpHeaders(), HttpStatus.MULTI_STATUS, request);
    }
    ...
    private ValidationErrorDTO processFieldErros(final List<FieldError> fieldErrors) {
